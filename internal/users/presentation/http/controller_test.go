...
func newRepositoryFake() *repositoryFake {
	return &repositoryFake{users: make(map[string]model.User)}
}

func (repo *repositoryFake) Create(_ context.Context, user model.User) error {
	repo.users[user.ID] = user
	return nil
}

func (repo *repositoryFake) GetByID(_ context.Context, id string) (model.User, error) {
...
func (repo *repositoryFake) List(context.Context) ([]model.User, error) {
...
func newHandler(repository *repositoryFake) http.Handler {
	...
	getUser := query.NewGetUserByID(repository)
	listUsers := query.NewListUsers(repository)
	controller := userhttp.NewUserController(createUser, getUser, listUsers)
...
func TestCreateUser(t *testing.T) {
	...
	if response.Header().Get("Location") != "/v1/users/user-1" {
		t.Fatalf("got Location %q", response.Header().Get("Location"))
	}
	var body map[string]any
	if err := json.NewDecoder(response.Body).Decode(&body); err != nil {
		t.Fatalf("decode response: %v", err)
	}
	if body["id"] != "user-1" || body["name"] != "Ada Lovelace" {
...
func TestCreateUserRejectsInvalidJSON(t *testing.T) {
	handler := newHandler(newRepositoryFake())
	request := httptest.NewRequest(http.MethodPost, "/v1/users", bytes.NewBufferString(`{"name":`))
	response := httptest.NewRecorder()
...
func TestCreateUserRejectsInvalidName(t *testing.T) {
	...
	if body.Error.Code != "invalid_name" {
		t.Fatalf("got error code %q", body.Error.Code)
	}
}

func TestGetUserNotFound(t *testing.T) {
	handler := newHandler(newRepositoryFake())
	request := httptest.NewRequest(http.MethodGet, "/v1/users/missing", nil)
	response := httptest.NewRecorder()
...
func TestGetUser(t *testing.T) {
	...
	request := httptest.NewRequest(http.MethodGet, "/v1/users/user-1", nil)
	response := httptest.NewRecorder()
