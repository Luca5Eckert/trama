...
func TestDependencyRule(t *testing.T) {
	...
	forbidden := map[string][]string{
		"application":  {"/internal/production/infrastructure", "/internal/production/presentation"},
		"domain":       {"/internal/production/application", "/internal/production/infrastructure", "/internal/production/presentation"},
		"presentation": {"/internal/production/infrastructure", "/internal/production/domain"},
	}
	...
	err := filepath.WalkDir(moduleRoot, func(path string, entry fs.DirEntry, walkErr error) error {
		...
		layer := layerFor(moduleRoot, path)
	...
	if err != nil {
		t.Fatalf("walk production module: %v", err)
	}
}
